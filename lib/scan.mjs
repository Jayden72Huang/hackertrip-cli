...
function detectMiniProgramProject(projectConfig, appJson, miniPkg, cloudFunctions) {
  const techs = new Set();
  const keywords = new Set();
  let name = '';
  let description = '';

  if (projectConfig || appJson) {
    techs.add('WeChat Mini Program');
    techs.add('JavaScript');
    keywords.add('mini program');
    keywords.add('wechat');
  }

  if (projectConfig) {
    const cfg = parseJson(projectConfig);
    if (cfg) {
      if (cfg.projectname) name = cfg.projectname;
      if (cfg.cloudfunctionRoot || cfg.cloudbaseRoot) techs.add('CloudBase');
      if (cfg.compileType === 'miniprogram') keywords.add('hackathon');
      const setting = cfg.setting || {};
      if (setting.es6 || setting.enhance) techs.add('ES6');
      if (setting.urlCheck === false) keywords.add('API');
    }
  }

  if (appJson) {
    const app = parseJson(appJson);
    if (app) {
      if (Array.isArray(app.pages) && app.pages.length) keywords.add('mobile');
      if (app.tabBar) keywords.add('app');
      const using = app.usingComponents || {};
      Object.keys(using).forEach((key) => {
        if (/tdesign/i.test(using[key]) || /tdesign/i.test(key)) techs.add('TDesign');
      });
    }
  }

  if (miniPkg) {
    const pkg = extractFromPackageJson(miniPkg);
    pkg.techs.forEach((t) => techs.add(t));
    if (pkg.name && !name) name = pkg.name;
    if (pkg.description) description = pkg.description;
  }

  if (cloudFunctions) {
    techs.add('CloudBase');
    techs.add('Cloud Functions');
    keywords.add('serverless');
  }

  return { techs: [...techs], keywords: [...keywords], name, description };
}
...
async function detectDirSignals(dir) {
  ...
  try {
    ...
    if (entries.includes('project.config.json') && entries.includes('miniprogram')) {
      signals.push('WeChat Mini Program');
    }
    if (entries.includes('cloudfunctions')) {
      signals.push('Cloud Functions');
      signals.push('CloudBase');
    }
...
export async function scanProject(dir) {
  ...
  // 9. WeChat mini program signals
  const projectConfig = await tryReadFile(join(dir, 'project.config.json'));
  const appJson = await tryReadFile(join(dir, 'miniprogram/app.json'));
  const miniPkg = await tryReadFile(join(dir, 'miniprogram/package.json'));
  let hasCloudFunctions = false;
  try {
    const entries = await readdir(join(dir, 'cloudfunctions'));
    hasCloudFunctions = entries.length > 0;
  } catch {
    hasCloudFunctions = false;
  }
  const mini = detectMiniProgramProject(projectConfig, appJson, miniPkg, hasCloudFunctions);
  mini.techs.forEach((t) => techStack.add(t));
  mini.keywords.forEach((k) => keywords.add(k));
  if (!projectName && mini.name) projectName = mini.name;
  if (!description && mini.description) description = mini.description;

  // 10. Extract keywords from descriptions and tech stack
