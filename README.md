...
配套还有 3 个 [Claude Code](https://claude.com/claude-code) skills，让 AI 帮你把上面的流程跑顺（见下文）。
...
```bash
hackertrip submit-event --from event.json
```
...
仓库 `.claude/skills/` 下提供 3 个 `HackerTrip-*` skill，让 Claude Code（或任意支持 skills 的 AI agent）端到端帮你跑完整流程——你只要用自然语言说需求，AI 会自动扫描、补全、调用上面的 CLI 命令。

| Skill | 给谁用 | 做什么 |
|---|---|---|
| **HackerTrip-publish-work** | 选手 | 扫 repo 自动抓作品信息 + 引导补全 demo/截图/获奖，经配对码安全同步到个人主页。说"帮我把黑客松作品发到 HackerTrip"即触发。 |
| **HackerTrip-submit-event** | 主办方 | 口述活动或丢一张海报图，AI 视觉 OCR 整理成标准字段 + 过准入门槛自检，提交到草稿箱。说"帮我把这个黑客松发到 HackerTrip"即触发。 |
| **HackerTrip-hackathon-intake** | 平台运营 | 多源抓取 + 核实 + 准入门槛（真黑客松/时效性/真实性/完整度）+ 入库草稿，批量采集第三方赛事用。 |

**怎么用**：把 `.claude/skills/` 下的 skill 目录放到你的 Claude Code skills 目录，对话里直接说目标（如"我想把作品发布到 HackerTrip"），Claude 会自动加载对应 skill 并引导你走完每一步。
